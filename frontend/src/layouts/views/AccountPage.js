...
function AccountPage() {
  const { updateUserInfo, info, isLoggedIn, username, password, IsAdmin, name, email, dept, phone, joinDate, loginDate, controlDate, log } = useAuth();
            ...
            <tbody>
              {Array.isArray(joinDate) ? (
                joinDate.map((date, index) => (
                  <tr key={index}>
                    <td>
                      <div>{new Date(date).toLocaleDateString('ko-KR').replace(/\//g, '.')}</div>
                    </td>
                    <td>
                      <div>{new Date(date).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: true })}</div>
                    </td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td>
                    <div>{new Date(joinDate).toLocaleDateString('ko-KR').replace(/\//g, '.')}</div>
                  </td>
                  <td>
                    <div>{new Date(joinDate).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: true })}</div>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <Table hover>
            <tbody>
              {Array.isArray(loginDate) ? (
                ...
                <tr>
                  <td>
                    <div>{new Date(loginDate).toLocaleDateString('ko-KR').replace(/\//g, '.')}</div>
                  </td>
                  <td>
                    <div>{new Date(loginDate).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: true })}</div>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <Table hover>
            <tbody>
              {Array.isArray(controlDate) ? (
                controlDate.map((date, index) => (
                  <tr key={index}>
                    <td>
                      <div>{new Date(date).toLocaleString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: true }).replace(/\//g, '.').replace(',', '')}</div>
                    </td>
                    <td>
                      <div>{log[index]}</div>
                    </td>
