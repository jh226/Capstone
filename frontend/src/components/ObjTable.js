...
function EObjTable({ active, userid }) {
  ...
  //기기 정보(on/off) 가져오기
  const getdevicelist_human = useCallback(async () =>{
    try {
  ...
  //tab 필터 처리
  const filterData = useCallback((active) => {
    let filteredData = [];
    if (active === "All") {
  ...
  }, [devices, devices_human]);

  useEffect(() => {
    filterData(active);
  }, [active, devices, devices_human, filterData]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        if (!devices.length || !devices_human.length) {
          await getdevicelist();
    ...
    fetchData();
  },[getdevicelist, getdevicelist_human, devices.length, devices_human.length]);
  ...
  const handleUpdateSave= async () => {
    if (
      ...
      const new_Latitude = parseFloat(formData.latitude1 +"." + formData.latitude2);
      const new_Longitude = parseFloat(formData.longitude1 +"." + formData.longitude2);
  ...
  const handleSave = async () => {
    if (
    ...
    else if(!isNumeric(document.getElementById("ip1").value) ||
      ...
      const new_latitude = parseFloat(document.getElementById("latitude1").value + "." +
        document.getElementById("latitude2").value);

      const new_longitude = parseFloat(document.getElementById("longitude1").value + "." +
        document.getElementById("longitude2").value);
  ...
  const Search = (e) => {
    ...
    if (!input) {
      ...
      const searchData = show_data.filter((item) => item.device_num.includes(input));
              ...
              <td>
                ...
                >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownItem onClick={() => handleShowDelete(item.device_num)}>Delete</DropdownItem>
                  </DropdownMenu>
                </Dropdown>
              </td>
            </tr>
        ...
        <Modal.Body className={styles.modal_content}>
          <Form>
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              ...
              <Row>
                <Col style={{ padding: "5px", width: "393px"}}>
                  <Form.Control type="text" value={formData.location} onChange={handlelocationChange} id="location" />
                </Col>
              </Row>
            </div>
